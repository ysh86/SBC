...
* Pico GPIO 25 = R/W in
* Pico GPIO 26 = VMA in
* Pico GPIO 27 = MR out (H 固定)
* Pico GPIO 28 = /RESET out
* Pico GPIO 29 = clock out 8MHz 固定

### ROM
Pico 内の 4KB 配列。初期値は別ヘッダファイルに hex dump の形式で書く。
MPU から見たアドレスは $F000-

### RAM
Pico 内の 56KB 配列。初期値は 0 埋め。
MPU から見たアドレスは $0000-

### UART
Pico の USB CDC を MPU に対して UART として提供する。
memory mapped I/O の 6850 互換で、$E000 がコントロール、$E001 がデータレジスタとする。
...
* R/W とアドレス値で ROM, RAM, UART を判定する
    ...
    * UART R: CDC で受信済みだったらその値をデータバスに出力、未受信だったら不定値を返す(block しない)
    * UART W: 待ち
